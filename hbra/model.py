...
# sparsesvd was replaced svds from scipy.sparse.linalg
...
class HBRA(object):
    ...
    def process(self, X):
        ...
        ut, s, vt = svds(norm_adj, hidden_dim)
        del ut, s
    ...
    def getUsersRating(self, batch_test, ds_name):  # 这个地方
        ret = 0
        for k, v in self.dict_matrix.items():
            for _k, _v in v.items():
                ...
                with torch.no_grad():
                    # if _v['norm_adj'].shape[0] != _v['norm_adj'].shape[1]:
                    linear_filter = _v['norm_adj'].T @ _v['norm_adj']
                    # else:
                    #     linear_filter = _v['norm_adj']
                    ...
                    #print(f'{batch_test[k].shape}1111111111111111111111111111111')
                    #print(f'{linear_filter.shape}2222222222222222222222222222222')

                    U1 = torch.mm(batch_test[k], linear_filter)
                    U2 = torch.mm(batch_test[k], torch.mm(left_matrix, right_matrix))
                    r = scalar * (U1 + 0.3 * U2)
                    if k == 'item' or k == 'item1':
                        r = r.transpose(1, 0)
...
class BSPM(object):
    ...
    def train(self):
        ...
        norm_adj = norm_adj.dot(d_mat)
        self.norm_adj = norm_adj.tocsc()
        ut, s, self.vt = svds(self.norm_adj, self.factor_dim)
        end = time.time()
